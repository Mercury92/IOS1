...
// 클래스는 참조 타입
// 타입이름은 대문자 카멜케이스로 정의
// 클래스의 타입 메서드는 두 종류(상속 후 재정의가 가능한 class 타입메서드, 상속 후 재정의가 불가능한 static 타입메서드)
class Sample {
    // 가변 프로퍼티
...
// 인스턴스 생성
let sueun: Student = Student()
sueun.name = "choisueun"
sueun.selfIntroduce() // 저는 Swift반 choisueun입니다
